...
BEGIN
	...
	select array_agg(id) into _ids from ( select distinct id from domesa.jsonb_to_vguia($1)) as r;

	with for_insert as (
		insert into domesa.guia(id, id_tipo_envio, id_forma_pago, id_operador, id_tipo_servicio, id_tipo_embalaje, id_status_guia, nro_guia, peso , domesa_monto ,domesa_costo_servicio ,conciliacion ,comision_porcentual ,recarga_x_embalaje ,tipo_traslado ,mercadolibre ,fecha_registro)
			select distinct on(id)
				  cte.id
				, cte.id_tipo_envio
				, cte.id_forma_pago
				, cte.id_operador
				, cte.id_tipo_servicio
				, cte.id_tipo_embalaje
				, cte.id_status_guia
				, cte.nro_guia
				, cte.peso
				, cte.domesa_monto
				, cte.domesa_costo_servicio
				, cte.conciliacion
				, cte.comision_porcentual
				, cte.recarga_x_embalaje
				, cte.tipo_traslado
				, cte.mercadolibre
				, cte.fecha_registro
			from domesa.jsonb_to_vguia($1) as cte
			where cte.for_updated is false
			ON CONFLICT (id) DO NOTHING
			returning guia.id
	) SELECT array_agg(id) into _created from for_insert;
	...
	RETURN QUERY select * from domesa.guia where ARRAY[id] <@ _ids;
END;
$$ LANGUAGE PLPGSQL;
*/
